...
extern memcg_sockets_enabled_key;
...
//		mbsFS_getpage(inode, start - 1, &page, SGP_READ);
//		if (page) {
...
//	if (partial_end) {
//		struct page *page = NULL;
//		mbsFS_getpage(inode, end, &page, SGP_READ);
//		if (page) {
...
static int mbsFS_fault(struct vm_fault *vmf)
{
	...
	error = mbsFS_getpage_gfp(inode, vmf->pgoff, &vmf->page, sgp,
			gfp, vma, vmf, &ret);
	if (error)
		...
		struct page **pagep, void **fsdata)
{
	...
	return mbsFS_getpage(inode, index, pagep, SGP_WRITE);
}
	static int
...
static ssize_t mbsFS_file_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	for (;;) {
		...
		error = mbsFS_getpage(inode, index, &page, sgp);
		if (error) {
			if (error == -EINVAL)
				error = 0;
			break;
...
static int mbsFS_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	if (len <= SHORT_SYMLINK_LEN) {
		...
		error = mbsFS_getpage(inode, 0, &page, SGP_WRITE);
		if (error) {
			iput(inode);
			return error;
		}
		...
		struct delayed_call *done)
{
	...
	if (!dentry) {
		...
		error = mbsFS_getpage(inode, 0, &page, SGP_READ);
		if (error)
...
MODULE_DESCRIPTION("mbsFS: A Persistent Memory File System mimic tmpfs");
MODULE_LICENSE("GPL");
/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
		...
		pgoff_t index, gfp_t gfp)
{
	...
	error = mbsFS_getpage_gfp(inode, index, &page, SGP_CACHE,
			gfp, NULL, NULL, NULL);
	if (error)
		page = ERR_PTR(error);
	else
