...
nova_info("%s : index= %d, nid = %d, size= 0x%lx (%ld) , er_list_blocks = 0x%lx (%ld)\n",
		__func__,index, nid, size, size,
...
static long nova_alloc_blocks_in_free_list_regions(struct super_block *sb,
...
static int nova_new_blocks_regions(struct super_block *sb, unsigned long *blocknr,
...
// blocknr.  Return the number of blocks allocated.
int nova_new_log_blocks_regions(struct super_block *sb,
